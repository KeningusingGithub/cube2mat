# features/std_volume.py
from __future__ import annotations
import datetime as dt
import pandas as pd
from feature_base import BaseFeature, FeatureContext

class StdVolumeFeature(BaseFeature):
    """
    09:30–15:59(交易所本地时区，默认 America/New_York) 内，
    按 symbol 计算 volume 的标准差；若该 symbol 当日有效点 < 3，则置为 NaN。
    输出列至少包含: ['symbol', 'value']。
    """
    name = "std_volume"
    description = "Intraday volume std between 09:30–15:59; NaN if <3 ticks."
    ...
    def process_date(self, ctx: FeatureContext, date: dt.date):
        ...
        df_intraday = df_intraday.dropna(subset=["volume"])
        if df_intraday.empty:
            out = sample[["symbol"]].copy()
            out["value"] = pd.NA
            return out
        ...
        g = df_intraday.groupby("symbol")["volume"]
