# quote_features/micro_dir_consistency_tw.py
...
class QuoteMicroDirConsistencyTWOnefileFeature(QuoteBaseFeature):
    """
    ...
    """
    name = "quote_micro_dir_consistency_tw_all"
    description = "RTH time-share of (microprice > mid) per symbol (onefile, single pass)"
    ...
    def process_date(self, ctx: FeatureContext, date: dt.date):
        ...
        for rb in pf.iter_batches(columns=cols, batch_size=self.BATCH_SIZE):
            ...
            if bool(first_mask.any()):
                ...
                prev_ts.loc[first_mask]  = np.array([last_ts_by.get(s)  for s in syms_first], dtype="float64")
                prev_val.loc[first_mask] = np.array([last_val_by.get(s) for s in syms_first], dtype="float64")
            ...
            agg = pd.DataFrame({"symbol": sub["symbol"].values, "w": w.values, "ws": ws.values}) \
                    .groupby("symbol", observed=True).agg(w_sum=("w", "sum"), ws_sum=("ws", "sum"))
        ...
        share_by = {k: (ws_by[k] / w) for k, w in w_by.items() if w > 0.0}
...
feature = QuoteMicroDirConsistencyTWOnefileFeature()
