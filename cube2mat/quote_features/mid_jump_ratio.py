# quote_features/mid_jump_ratio.py
...
class QuoteMidJumpRatioOnefileFeature(QuoteBaseFeature):
    """
    ...
    """
    name = "quote_mid_jump_ratio_all"
    description = "RTH event share of |Δmid|/mid_prev > tau per symbol (onefile, single pass)"
    default_quote_root = str(DATARAW_ROOT / "us" / "quote_onefile")

    RTH_START = dt.time(9, 30)
    RTH_END = dt.time(16, 0)
    BATCH_SIZE = 500_000
    DEFAULT_JUMP_TAU = 5e-4
    ...
    def process_date(self, ctx: FeatureContext, date: dt.date):
        ...
        tau = float(getattr(ctx, "jump_tau", self.DEFAULT_JUMP_TAU))
        ...
        jump_by: Dict[str, int] = defaultdict(int)
        ...
        for rb in pf.iter_batches(columns=cols, batch_size=self.BATCH_SIZE):
            ...
            if bool(valid_pair.any()):
                rel = np.abs(sub["mid"].values[valid_pair] - p_mid.values[valid_pair]) / p_mid.values[valid_pair]
                is_jump = (rel > tau).astype(int)
                syms = sub["symbol"].values[valid_pair]
                tmp = pd.DataFrame({"symbol": syms, "j": is_jump, "c": 1}).groupby("symbol", observed=True).sum()
                for sym, row in tmp.iterrows():
                    jump_by[sym] += int(row["j"])
                    cnt_by[sym] += int(row["c"])
        ...
        ratio_by = {k: (jump_by[k] / c) for k, c in cnt_by.items() if c > 0}

        out = sample[["symbol"]].copy()
        out["value"] = [ratio_by.get(str(s), pd.NA) for s in sample["symbol"]]
        return out


feature = QuoteMidJumpRatioOnefileFeature()
