# quote_features/size_imbalance_tw.py
...
class QuoteSizeImbalanceTWOnefileFeature(QuoteBaseFeature):
    """
    ...
    """
    name = "quote_size_imbalance_tw_all"
    description = "RTH time-weighted mean of (bid_size-ask_size)/(bid_size+ask_size) per symbol (onefile, single pass)"
    ...
    def process_date(self, ctx: FeatureContext, date: dt.date):
        ...
        cols = ["symbol", "ask_size", "bid_size", "participant_timestamp"]
        ...
        for rb in pf.iter_batches(columns=cols, batch_size=self.BATCH_SIZE):
            ...
            sub = pd.DataFrame({
                ...
                "val":    imb.loc[valid_now].values.astype(np.float64),
            })
...
feature = QuoteSizeImbalanceTWOnefileFeature()
